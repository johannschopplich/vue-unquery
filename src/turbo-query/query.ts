import type { TurboCache } from './cache'
import { createTurboCache } from './cache'
import { createTurboEvents } from './events'
import type { TurboEvents, TurboListener } from './events'
...
export function createTurboQuery(instanceOptions?: TurboQueryConfiguration): TurboQuery {
  ...
  function mutate<T = any>(
    ...
    if (typeof item === 'function') {
      ...
      const cached = hasKey
        ? itemsCache.get<ItemsCacheItem<T>>(key)
        : undefined
  ...
  function abort(cacheKeys?: string | string[], reason?: any): void {
    const resolverKeys = typeof cacheKeys === 'string'
      ? [cacheKeys]
      : cacheKeys ?? keys('resolvers')
    ...
    async function refetch(key: string): Promise<T> {
      try {
        ...
        expiresAt.setMilliseconds(
          expiresAt.getMilliseconds() + expiration(item),
        )
