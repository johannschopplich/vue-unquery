...
import { createTurboQuery } from '../../src/turbo-query/query'
import { createTurboCache } from '../../src/turbo-query/cache'
import { createTurboEvents } from '../../src/turbo-query/events'
import type { TurboFetcherAdditional } from '../../src/turbo-query/query'
...
it.concurrent(
  'can fetch expired resources while returning stale',
  async ({ expect }) => {
    async function fetcher() {
      return 'example'
    }
...
it.concurrent(
  'can fetch expired resources while not returning stale',
  async ({ expect }) => {
    async function fetcher() {
      return 'example'
    }
...
it.concurrent(
  'returns the same promise when resources are resolving',
  async ({ expect }) => {
    let times = 0
    async function fetcher() {
...
it.concurrent(
  'does respect dedupe interval of resources',
  async ({ expect }) => {
    let times = 0
    async function fetcher() {
      times++
...
it.concurrent(
  'does respect dedupe interval of resources 2',
  async ({ expect }) => {
    let times = 0
    async function fetcher() {
      times++
...
it.concurrent(
  'can subscribe to refetchings on resources',
  async ({ expect }) => {
    async function fetcher() {
      return 'example'
    }

    const { query, subscribe } = createTurboQuery({
      fetcher,
      expiration: () => 0,
    })
...
it.concurrent(
  'can subscribe to refetchings on pending resources',
  async ({ expect }) => {
    async function fetcher() {
      return 'example'
    }
...
it.concurrent(
  'can subscribe to resolutions on resources',
  async ({ expect }) => {
    async function fetcher() {
      return 'example'
    }
...
it.concurrent(
  'can mutate non-existing cache keys when using a fn',
  async ({ expect }) => {
    async function fetcher() {
      return 'example'
    }
...
it.concurrent(
  'can subscribe to mutations on resources 2',
  async ({ expect }) => {
    async function fetcher() {
      return 1
    }
...
it.concurrent('can subscribe to aborts on resources', async ({ expect }) => {
  ...
  const { query, subscribe, abort } = createTurboQuery({
    fetcher,
    expiration: () => 0,
  })
...
it.concurrent(
  'can get the resolvers keys of a turbo query',
  async ({ expect }) => {
    async function fetcher() {
...
it.concurrent('removes resolver when query fails', async ({ expect }) => {
  ...
  await expect(query<string>('example-key', { fetcher })).rejects.toThrowError(
    'foo',
  )
  await expect(
    query<string>('example-key', { fetcher: fetcher2 }),
  ).resolves.toBe('example')
})

it.concurrent(
  'removes items if specified when query fails',
  async ({ expect }) => {
    async function fetcher(): Promise<string> {
      throw new Error('foo')
    }
...
it.concurrent('can subscribe to errors', async ({ expect }) => {
  ...
  subscribe<Error>('example-key', 'error', (e) => {
    err = e
  })
...
it.concurrent('can hydrate keys', async ({ expect }) => {
  ...
  const { query, hydrate } = createTurboQuery({
    fetcher,
    expiration: () => 1000,
  })
